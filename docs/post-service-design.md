...
```
       ...
       [mobile-gateway]   ── JWT 解出 user_id,注入 gRPC Metadata ──┐
                ...
                ▼                                                    │
   ┌──────────────────────────────────────────────────────────┐     │
   │  post-service  (Python 3.11+ / grpcio + APScheduler)       │     │
   │  - 帖子读写 / 删除                                         │     │
...
- 计数刷盘走 Redis Set + APScheduler(见 §6.2 写合并),不进 MQ。
...
部署:本机 Jenkins 一条流水线 `uv build` / `docker build` → `docker compose up -d`,容器跑在本机 docker 网络 `dating-app` 里,中间件全部连远端 `38.76.188.242`(见 `student-dev-guide §4 / §9.1`)。
...
| 类别 | 选型 | 备注 |
|---|---|---|
| 语言 / 运行时 | Python 3.11+ | 容器用 `python:3.11-slim` |
| gRPC 框架 | `grpcio` 1.68+ / `grpcio-tools` | 纯 gRPC 服务,不带 HTTP 业务入口 |
| 轻量 HTTP(健康检查 / metrics) | `fastapi` + `uvicorn`(可选) | 仅用于 `/health`、`/metrics`,不暴露业务接口 |
| ORM | SQLAlchemy 2.0 (`sqlalchemy[asyncio]` + `asyncpg`) | 单表查询;**禁多表 JOIN**(红线 1) |
| DB | PostgreSQL 16 | 列统一 `TIMESTAMPTZ`,连接 `SET TIME ZONE 'UTC'`(红线 8) |
| 迁移 | Alembic | 本服务自己的 migration 目录 `migrations/versions` |
| 缓存 | Redis 7 | key 前缀 `<name>:`(见 `student-dev-guide §6.2`);客户端用 `redis.asyncio.Redis` |
| 分布式锁 / 去重 | Redis `SET NX EX` 或 RedisBloom | 简单锁用 `redis.lock`;布隆过滤用 RedisBloom 模块或 `pybloom-live` |
| 定时任务多实例互斥 | APScheduler + PostgreSQL job store(`max_instances=1`) | 本机单实例可接内存 job store;部署多实例必须共享 PG job store |
| 对象存储 | MinIO,`dating-common` 的 `ObjectStorage` 接口 | bucket `dating-<name>`,`path-style-access: true`(`student-dev-guide §6.3`) |
| 配置 / 注册 | Nacos 2.4(`nacos-sdk-python`) | namespace `dev-<name>` |
| gRPC 服务发现 | `grpcio` 自定义 resolver 或 Nacos 拉地址列表 | 服务发现 `user-service`(同 namespace) |
| MQ | `rocketmq-client-python`(RocketMQ 5.x) | **仅 fanout 场景**;topic / group 必带 `youjianxin-dating-dev-` 前缀(CLAUDE.md 隔离前缀) |
| Proto 依赖 | Nexus PyPI `dating-proto-<name>-post==0.1.0` | 同版本号只能发一次 |
| 序列化 | `google.protobuf.json_format.MessageToDict` | 如需要 JSON 出参直接转 dict |
| 本地缓存 | `cachetools.TTLCache` | UserClient 性别缓存 30s |
| 指标 | `prometheus-client` | Counter / Histogram / Gauge |
| 日志 | Python 标准 `logging` | 结构化可选 `structlog`;禁写文件,stdout → Promtail → Loki |
...
照 `student-dev-guide §5` 模板,这里给本服务实际模块布局:

```
post_service/
├── server.py                         # 启动 gRPC server + APScheduler + 可选 HTTP metrics
├── config.py                       # pydantic-settings,读环境变量 / Nacos
├── grpc_server/
│   └── post_servicer.py            # 9 个 RPC 实现,只编排 service
├── services/                       # 业务编排 + 事务边界
│   ├── post_write_service.py       # 发帖 / 删帖
│   ├── post_read_service.py        # 详情 / 用户帖子列表
│   ├── like_service.py             # 点赞 upsert + Redis 增量
│   ├── comment_service.py          # 评论 增 / 删 / 列
│   └── feed_service.py             # 池重建 + 三路混合读
├── mq/
│   ├── fanout_producer.py          # 发帖事务后 sync_send 到 fanout topic
│   └── fanout_consumer.py          # 消费 fanout msg,拉 followers 后 ZADD timeline
├── jobs/                           # APScheduler 定时任务
│   ├── like_flush_job.py           # 每分钟刷盘点赞
│   ├── comment_flush_job.py        # 每分钟刷盘评论
│   └── feed_score_job.py           # 每 5 分钟重建池
├── managers/                       # 单聚合 + 缓存读写
│   ├── post_manager.py             # post + post_image 写
│   ├── post_stat_manager.py        # post_stat 增量更新
│   ├── post_like_manager.py        # post_like upsert
│   └── post_comment_manager.py
├── repositories/                   # 一张表一个 repo
│   ├── post_repository.py
│   ├── post_image_repository.py
│   ├── post_stat_repository.py
│   ├── post_like_repository.py
│   └── post_comment_repository.py
├── model/                          # SQLAlchemy declarative models,表 1:1
│   ├── post_model.py
│   ├── post_image_model.py
│   ├── post_stat_model.py
│   ├── post_like_model.py
│   └── post_comment_model.py
├── clients/
│   └── user_client.py              # 调 user-service gRPC stub
├── config/
├── constants/                      # PostStatus / LikeStatus / ErrorCode
└── exceptions/                     # BizException + grpc interceptor 兜底
```

调用方向严格单向:`grpc_server → services → managers → repositories`,`jobs → services / managers`,`mq.consumer → services / managers`(红线 10)。
...
- `deleted SMALLINT DEFAULT 0` 配合 SQLAlchemy 软删除过滤器。
...
配置 `migrations/env.py` 把 `sqlalchemy.url` 指向 `config.DATABASE_URL`,并在 `target_metadata` 注册所有 models。

新建 migration:

```bash
uv run alembic revision -m "init post tables"
# 在生成的 migration 里手写 5 张表 + 索引 + shedlock 表
```
...
**全表 key 必须带 `<name>:` 前缀**,通过 `config.CACHE_KEY_PREFIX` 统一注入,业务代码用变量拼,不要散落硬编码(`student-dev-guide §6.2`)。
...
| `<name>:user:read:bloom:{user_id}` | BloomFilter(容量 5000,误判 1%)| 7d | 已读去重 |

> ⚠️ 还有一张 `shedlock` 表(PG 不在本表),用于多实例部署时 `LikeFlushJob` / `CommentFlushJob` / `FeedScoreJob` 互斥。它**不是 Redis key**,本机单实例开发可以不接;部署多实例前必须接(否则 Job 重复跑浪费 PG 资源)。原理见 §6.5。
...
### 6.5 APScheduler:多实例定时任务互斥

本服务有 3 个 Job(`LikeFlushJob` / `CommentFlushJob` / `FeedScoreJob`),由 APScheduler 调度。**单实例部署完全不需要 PG job store**,用 `MemoryJobStore` 就行。

部署 2 个及以上实例时(高可用 / 灰度发布并存)就有问题:两个实例都按 `interval=60` 秒触发,会**重复跑同一个 Job**:
...
不致命,但浪费 + 难排查。**APScheduler + PostgreSQL job store + `max_instances=1` 保证同名 Job 同一时刻全集群只有一个实例跑**。
...
@scheduler.scheduled_job("interval", seconds=60, max_instances=1, coalesce=True)
async def flush_likes() -> None:
    ...
```

#### 它的存储:PG job store 内部表

APScheduler 的 `SQLAlchemyJobStore` 会自动建 `apscheduler_jobs` 表(包含 `id`、`next_run_time`、`job_state` 等字段),利用行级锁保证多实例互斥。

抢锁流程(由 APScheduler 自动做):

```
1. 实例 A: UPDATE apscheduler_jobs SET next_run_time = ... WHERE id='flush_likes' AND next_run_time <= now
           成功 → 拿锁,跑 Job
2. 实例 B (同一秒触发):
           同一行已被 A 锁占用或 next_run_time 已推后 → 0 行更新 → 跳过本次
3. A 跑完: 更新 next_run_time = now + interval → 释放
4. 1 分钟后 B 再触发,时间到了 → 成功抢到 → B 跑
```

#### 为什么选 PG job store

- 学员服务已经依赖 PG,**少一个组件依赖**。
- 锁状态在 PG 也方便 `SELECT * FROM apscheduler_jobs` 排查。
- 每分钟只抢一次锁,开销可以忽略。

> 学员阶段开发期单实例跑,**可以先接 MemoryJobStore 不加 PG job store**。部署到 docker compose 同时跑 2 个 replicas 之前再切到 `SQLAlchemyJobStore`。
...
4. **将来真要切到团队多桶模式**,只需要把 endpoint + bucket 名换到不同环境,业务代码里**只动 `config.py` 那一行 `MINIO_BUCKET`**,因为本服务始终只用 `post-image/` 前缀 —— 换桶等价于换路径根。
...
- gRPC 出参一律回 `image_key`(字段名带 `_key`)。
...
- 私有 / 敏感资产 → 在 post-service 的 proto 里新增一个 RPC(例 `GetPostImageBytes(post_id, sort_order) → bytes` 或 `BatchPresignPostImages`),由 post-service 走 `ObjectStorage.get_object` 读出后透传 bytes / 签 presigned GET URL。

**关键**:调用方**绝不绕过 post-service 直接以 `post-image/...` 前缀 `get_object`**。原因:
...
proto 放在 workspace 根的 `proto/post/post.proto`,在 `proto/` 下生成 Python gRPC 代码并打包上传到 Nexus PyPI,包名 `dating-proto-<name>-post==0.1.0`(`student-dev-guide §6.5`)。
...
**统一返回包装**:proto 里 `BaseResponse{code, message, extra}`。业务异常由 grpc interceptor 兜底转 `BaseResponse(code != 0)`。gRPC 状态码统一 OK,业务错误靠 code 区分。

**user_id 透传方式**:`mobile-gateway` 解 JWT 得到 user_id 后塞进 gRPC Metadata(key=`x-user-id`),本服务用 grpc interceptor 在每个 RPC 入口取出来注入 `contextvars` / log 上下文。**禁止把 user_id 作为业务字段塞 proto request 里**(除非是「操作目标」如 `ListUserPosts` 的 `user_id` 是查谁的帖子,这是合法用法)。
...
业务异常由 grpc interceptor 兜底:`BizException` → proto `BaseResponse{code, message}`,gRPC 层统一 OK。
...
App ─ CreatePost(content, [image_key]) ─→ post-service
   ...
   5. ZADD <name>:feed:cold_start:pool:{发帖人性别}
      (gender 由 UserClient.get_gender_by_user_id 拿,带 cachetools 30s 本地缓存避免热点)
   6. PostFanoutProducer.sync_send(topic=youjianxin-dating-dev-post-fanout-v1,
...
1. SELECT 校验:存在 + 未删 + `user_id == post.user_id`(权限)
2. UPDATE `posts.deleted = 1`
3. DEL `<name>:post:detail:{post_id}`
4. ZREM 冷启动池里这条
5. SREM `<name>:post:updated_set`

> ⚠️ 不删 `post_likes` / `post_comments` 历史(留审计),不删 `user:timeline:*` 里的 member(成本太高;读侧拿到死 post_id 时 `get_post_detail` 抛 `POST_NOT_FOUND`,FeedService try-except warn 后跳过)。

### 9.3 点赞 / 取消(ActionLike)

PostgreSQL upsert(用 SQLAlchemy `on_conflict_do_update`):
...
### 9.4 LikeFlushJob(每 1 分钟,APScheduler 多实例互斥)

```
1. SRANDMEMBER <name>:post:updated_set 100
2. for each post_id:
...
- APScheduler `max_instances=1` + PG job store 防多实例同时跑导致重复 SPOP。

### 9.5 创建评论(CreateComment)

1. validate_comment(content,1-512,trim)
...
1. ZSet 有数据 → zrevrangebyscore(zsetKey, -inf, N-1, 0, pageSize)
2. 拿到 comment_id 列表 → 逐个 get_by_id(命中 Redis 缓存里都是热数据)
...
gender = user_client.get_gender_by_user_id(currentUserId)
opposite_sex = not gender   // 异性优先

并行三路:
  ① ZREVRANGE  <name>:feed:pool:recommend:{opposite_sex}      [recOffset, recOffset+30]
  ② ZREVRANGEBYSCORE  <name>:user:timeline:{currentUserId}    最近 7 天,0,5
  ③ ZREVRANGE  <name>:feed:cold_start:pool:{opposite_sex}     [csOffset, csOffset+10]

布隆过滤:
  BloomFilter <name>:user:read:bloom:{currentUserId}
  → 三路所有 ID 过滤一遍

混排(FeedService.merge_three_way):
...
拼装:
  for each post_id: post_read_service.get_post_detail(...) (try-except 跳过已删)
  for each returned id: bloom.add(str(id))
...
```
Score = (W_base + α · like_count + β · comment_count) / (hours_since_published + 2) ^ 1.5
...
|---|---|---|---|---|
| ① 全网热门池 `feed:pool:recommend:{gender}` | `FeedScoreJob` APScheduler 5min | 主力填充大多数位置 | Hacker News 热度分 | ✅ |
...
**触发**:`FeedScoreJob` APScheduler interval 300 秒,每 5 分钟全量重建。多实例部署靠 APScheduler PG job store + `max_instances=1` 互斥 (§6.5)。
...
│                                                              │
│  post_stats(批量):                                          │
│    session.execute(select(PostStat).where(PostStat.post_id.in_(post_ids)))
│    // WHERE post_id IN (...) 单表查                          │
└──────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌── Step 2: 实时计数补偿(读 Redis 增量)──────────────────────┐
│  for each post_id:                                          │
│    like_incr    = GET <name>:post:stat:incr:{pid}:likes     │
│    comment_incr = GET <name>:post:stat:incr:{pid}:comments  │
│    likes    = stat.like_count    + like_incr                │
│    comments = stat.comment_count + comment_incr             │
...
│  for each post:                                             │
│    hours_diff = (now - created_at_epoch) / 3600.0           │
│    score = (10 + 1.0 * likes + 3.0 * comments)              │
│              / (hours_diff + 2) ** 1.5                      │
└──────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌── Step 4: 性别分桶(批量 RPC + cachetools 30s 缓存) ────────┐
│  distinct_user_ids = {p.user_id for p in posts}             │
│  gender_map = user_client.get_genders(distinct_user_ids)    │
│  // ⚠️ 必须批量!单条调 N 次会把 user-service 压垮            │
│  // 本地 cachetools 30s 缓存吸收同一用户的多帖重复查询         │
│                                                              │
│  for each post:                                             │
│    if gender_map[post.user_id] == MALE:                     │
│        zadd_batch["...:male:tmp"].append((score, post_id))  │
│    else:                                                    │
│        zadd_batch["...:female:tmp"].append((score, post_id))│
└──────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
- Hacker News 公式有时间衰减项 `(hours_diff + 2)^1.5`,意思是**每条帖的分数随时间被动跌**,即使没人点赞。要实时维护就得跟着秒级时钟跑,所有 3000 个 post_id 每秒重排,得不偿失。
...
**失败模式**:Job 卡死 → 池仍是上次成功重建的快照,读侧不报错,只是数据陈旧。监控 `feed.score.rebuild.duration` Histogram + alert if 超过 2 分钟。
...
**为什么用 MQ 而不是 `asyncio.create_task`**(v0.3 调整,前置评审通过):

| 维度 | `asyncio.create_task`(原方案) | RocketMQ(现方案) |
|---|---|---|
| 进程崩溃 | 事件循环里未跑的任务直接丢 | Broker 持久化,Consumer 重启继续 |
| 关注者多 | 单协程 ZADD N 次,阻塞 loop | Consumer 并发拉,多实例分担 |
| 失败重试 | 抛异常即丢 | 自动重投 16 次 → 仍失败转 DLQ |
| 扩容吞吐 | 加 loop 任务(仍单进程) | 加 Consumer 实例,水平扩展 |
| 跟发帖事务关系 | 同进程事务后调度 | 事务 COMMIT 后 sync_send,失败不回滚 DB |

**触发**:`PostWriteService.create_post` 事务 COMMIT 之后,同步调 `PostFanoutProducer.sync_send` 发到 topic `youjianxin-dating-dev-post-fanout-v1`(§9.1 第 6 步)。
...
- 同一作者多帖 fanout 时,Consumer 端的 cachetools 30s 缓存还能命中
...
FAIL = Counter("post_fanout_produce_fail", "Producer 重试 3 次仍失败")
...
class PostFanoutProducer:
    ...
    def send(self, post_id: int, author_user_id: int, created_at_epoch: int) -> None:
        body = json.dumps({
            "postId": post_id,
        ...
        for i in range(3):
            try:
                result = self._producer.send_sync(msg, timeout=2)
                if result.status == SendStatus.OK:
                    return
            except Exception as e:
                logger.warning("fanout send retry, post_id=%s attempt=%s", post_id, i + 1, exc_info=e)
...
GENDER_CACHE: TTLCache = TTLCache(maxsize=10_000, ttl=30)

class PostFanoutConsumer:
    def __init__(self, user_client: UserClient, redis: Redis) -> None:
        self._user_client = user_client
        self._redis = redis

    def consume(self, msg) -> ConsumeStatus:
        data = json.loads(msg.body)
        author_id = data["authorUserId"]
        post_id = data["postId"]
        epoch = data["createdAtEpoch"]

        # user-service down → 抛 RPC 异常 → RocketMQ 自动重投,给上游喘息时间
        followers = self._user_client.get_friend_user_ids(author_id)
        if not followers:
            return ConsumeStatus.CONSUME_SUCCESS

        for follower in followers:
            key = redis_key.user_timeline(follower)
...
│  producer.send_sync(msg, {postId, authorUserId, ts})         │
│    本地 retry 3 次,2s timeout/次                              │
│    全失败 → log.error + post.fanout.produce.fail +1           │
└──────────────────────────────────────────────────────────────┘
                              │  毫秒级,Broker 持久化
                              ▼
┌── Step 2: Consumer(同进程内,并发消费)───────────────────────┐
│  followers = user_client.get_friend_user_ids(author_id)     │
│    cachetools 30s 缓存吸收同作者多帖的重复 RPC                 │
...
**为什么 Consumer 并发消费而不是顺序消费**:

- 同一帖的 fanout 只有一条 msg,自身没有顺序问题
- 不同帖之间,timeline 排序由 ZSet `score = epoch` 决定,跟消费顺序无关
- 并发消费提高吞吐,关注数大的发帖人不阻塞别人

**为什么 Consumer 不用 APScheduler 互斥**:

RocketMQ Consumer Group 天然按 queue 分配,多实例自动负载均衡且不会重复消费同一条 msg。APScheduler 互斥只用于定时 Job(`LikeFlushJob` / `CommentFlushJob` / `FeedScoreJob`,见 §6.5)。
...
**目的**:解决「新帖刚发出来 like_count=0,Hacker News 公式打出来的分排不进 Top 3000,永远没人看到」的马太效应。冷启动池**不按热度排只按时间**,任何新帖立刻入池。

**触发**:`PostWriteService.create_post` 流程的第 5 步,事务提交后**同步** ZADD(只是一个 ZADD,毫秒级,没必要走 MQ;走 MQ 反而让"新帖立刻有曝光位"变成"几秒后才有",破坏 §10.2.3 的核心卖点)。

**完整构造流程**:

```
┌── Step 1: 判发帖人性别 ─────────────────────────────────────┐
│  is_male = user_client.is_male(发帖人 userId)               │
│  // 命中 cachetools 30s 缓存,~微秒                           │
│                                                              │
│  key = is_male ? <name>:feed:cold_start:pool:male           │
...
**频控**:同一好友单次最多 1 条(`used_friend_user_ids` set 维护),防 1 个话痨好友刷屏。
...
| 某 post_id 取出来时已被删 | `get_post_detail` 抛 BizException,catch warn 跳过(该页可能不足 10 条) |
...
本服务**唯一**外部 gRPC 依赖:`user-service`,通过 Nacos 解析地址列表。

```python
import grpc
from dating_proto_qidian_user.v1 import user_service_pb2, user_service_pb2_grpc

class UserClient:
    def __init__(self, channel: grpc.Channel) -> None:
        self._stub = user_service_pb2_grpc.UserServiceStub(channel)

    # ① 取好友 user_id 列表,用于发帖写扩散
    def get_friend_user_ids(self, user_id: int) -> list[int]: ...

    # ② 取性别,用于 Feed 池分桶
    #    返回:true=男 / false=女 / fallback false(user-service 不可用 / 用户未设置)
    def is_male(self, user_id: int) -> bool: ...

    # ③ 批量取性别,FeedScoreJob 重建池时用
    def get_genders(self, user_ids: list[int]) -> dict[int, bool]: ...
```

**关键约束**:

1. **本服务不缓存 user-service 返回的资料到 Redis**(`user-service` 自己已有 `user:profile:*` 缓存,二级缓存只会让一致性更难)。
2. **但允许本地短 TTL cachetools**(30 秒):`is_male` / `get_genders` 在 `FeedScoreJob` 重建池时会被 3000+ 次调用,RPC 风暴用 cachetools 削峰。cachetools 是进程内,挂了也不影响数据一致性。
3. **user-service 不可用降级**:`get_friend_user_ids` 返空(写扩散 no-op,用户只走全网池);`is_male` 默认 false(归到女性池,可接受不准)。绝不因 user-service 抖动让发帖 / Feed 失败。

> 如果你的 `user-service` 当前没实现「好友列表」「性别字段」,本服务**先用桩实现**(get_friend_user_ids 返空、is_male 用 `user_id % 2 == 0` 测试用),等 `user-service` 实现后无缝替换 stub 即可。
...
| 多实例 LikeFlushJob 同时跑 | post_stats 重复加 / 漏加 | Lua「GET + SET 0」原子 + `UPDATE += delta` + APScheduler `max_instances=1` |
...
| 删帖后 timeline / 池里残留 | 读到 404 | `get_post_detail` 抛 BizException,FeedService try-except 跳过 |
| user-service RPC 失败 | 写扩散 / 性别分桶失败 | UserClient fallback;不让发帖 / Feed 失败 |

**事务红线**(`student-dev-guide §7.4` / 红线 9):
- `async with session.begin()` 只在 service 方法,不嵌套远程调用。
...
- Python `logging` 打 stdout,**禁写文件**;本机 Promtail → Loki,Grafana `http://localhost:3000` 查询。
- ERROR 必带堆栈:`logger.error("xxx failed, post_id=%s", post_id, exc_info=True)`。
- 关键链路日志带 `trace_id` / `user_id`(contextvars / log 上下文透传)。
- 关键 keyword(grep 友好):
...
| 指标 | 类型 | 含义 |
|---|---|---|
| `post_create_total{status="success/fail"}` | Counter | 发帖成功率 |
| `post_like_action_total{action="LIKE/UNLIKE"}` | Counter | 点赞行为分布 |
| `post_like_flush_batch_size` | Histogram | 每次刷盘批量 |
| `feed_recommend_duration_seconds` | Histogram | Feed 读耗时 |
| `feed_recommend_bloom_hit_total` | Counter | 被布隆过滤掉的 |
| `feed_score_rebuild_duration_seconds` | Histogram | 重建池耗时 |
| `post_updated_set_size` | Gauge | 待刷盘队列长度(报警阈值 1 万) |
| `user_client_rpc_fail_total{method="..."}` | Counter | 跨服务调用失败 |
| `post_fanout_produce_fail_total` | Counter | Producer 重试 3 次仍失败的发帖数(每帖至多 +1)|
| `post_fanout_consume_latency_seconds` | Histogram | 单条 fanout 消息处理耗时 |
| `post_fanout_dlq_size` | Gauge | DLQ 积压数,>0 立即人工排查 |
...
- 黑名单 / 拉黑:由 `user-service` 统一管;`FeedService.merge_three_way` 加过滤 hook。
...
- [ ] 在自己 `<yourpinyin>-workspace` 仓库下,新建 `post-service` 目录,包名 `post_service`(`student-dev-guide §A`)
- [ ] `proto/post/post.proto` 编 9 个 RPC + 5 个 message;生成 Python gRPC 代码并打包 `dating-proto-<name>-post==0.1.0` 上传到 Nexus PyPI
- [ ] `post-service/pyproject.toml` 加依赖:
  - `dating-proto-<name>-post==0.1.0`
  - `grpcio` + `grpcio-tools`
  - `sqlalchemy[asyncio]` + `asyncpg`
  - `alembic`
  - `redis[hiredis]`
  - `cachetools`
  - `rocketmq-client-python`(fanout 用,选与本地 RocketMQ 版本兼容版本)
  - `apscheduler`
  - `prometheus-client`
  - `pydantic-settings`
  - `fastapi` + `uvicorn`(可选,仅 health/metrics)

### Day 3:数据层

- [ ] Alembic migration:5 张表 + 索引 + `shedlock`(若选手写自定义锁表)或直接 APScheduler PG job store 表
- [ ] 在 Nacos `dev-<name>` namespace 配 `post-service-dev.yaml`,Redis/PG/MinIO/Snowflake worker-id 都从这里读
- [ ] 5 个 SQLAlchemy models + 5 个 repositories(只在 `post_like` repo 里实现 upsert)

### Day 4 ~ 5:核心业务

- [ ] Managers(单表读写 + Redis 操作)
- [ ] Services:`PostWriteService` / `PostReadService` / `LikeService` / `CommentService`
- [ ] `BizException` + `ErrorCode` + grpc interceptor 全局异常处理
- [ ] 先在 service 层单元测试跑通增 / 查 / 删 / 点赞 / 评论;集成测试留到 Day 6 用 `grpcurl` 测

### Day 6:gRPC + Feed

- [ ] `PostServicer` 实现 9 个 RPC
- [ ] `UserClient`(先用桩,get_friend_user_ids 返空、is_male 取模)
- [ ] `mq/fanout_producer.py`(`sync_send` + 本地 3 次 retry + `post_fanout_produce_fail_total` 指标)
- [ ] `mq/fanout_consumer.py`(PushConsumer + 拉 followers + ZADD/裁剪/TTL)
- [ ] Nacos `post-service-dev.yaml` 加 RocketMQ name-server 配置(从 `dev-onboarding.md` 取地址)
- [ ] `FeedService.rebuild_recommend_pool` + `get_recommend_feed` + `merge_three_way`
- [ ] `jobs/like_flush.py` / `jobs/comment_flush.py` / `jobs/feed_score.py`(APScheduler)
- [ ] 用 `grpcurl` 测 gRPC:发帖 → 点赞 → 评论 → 刷盘 → Feed

### Day 7:接入 + 部署

- [ ] 在你的 `mobile-gateway` 加 `/post/*` 路由,JWT → user_id 注入 gRPC Metadata
- [ ] 写 `Dockerfile`(`python:3.11-slim`,多阶段可选,`uv sync` 安装依赖)
...
- [ ] 集成测试:pytest + Testcontainers 起真 PG + Redis 跑端到端
- [ ] `user-service` 实现 `GetFriendList` / `GetProfile.gender` 后替换 UserClient 桩
- [ ] 接 `/metrics` + Prometheus 暴指标
...
| # | 红线 | 本设计如何遵守 |
|---|---|---|
| 1 | 多表 JOIN | 所有 repositories 单表;评论列表 / Feed 详情靠 service 多次单表 + `where(...in_(...))` |
| 2 | 跨服务直连别人库 / Redis / 桶 | 好友 / 性别走 `UserClient` gRPC;workspace 共享桶但**只写自己的 `post-image/` 前缀,不读别的服务的 key 前缀** |
| 3 | 服务间 HTTP 互调 | post-service 与 user-service 全 gRPC,Nacos 寻址 |
| 4 | 凭据进 git | `config.py` / `.env` 全部读环境变量;真值进 Nacos(workspace `nacos/post-service-dev.yaml` 是共享 dev 凭据的粘贴模板,允许带真值,见 CLAUDE.md 红线 #1)或本机环境变量;生产凭据仍严禁进 git |
...
| 8 | `TIMESTAMP` 或写死 `Asia/Shanghai` | 全 `TIMESTAMPTZ`,asyncpg 连接初始化 `SET TIME ZONE 'UTC'`,容器 `TZ=UTC` |
| 9 | 跨服务事务 | `async with session.begin()` 内不嵌远程调用;写扩散经 MQ 在事务外,Producer 失败不回滚 |
| 10 | grpc_server 直调 repository | 严格 `grpc_server → services → managers → repositories` 单向;定时任务在独立 `jobs/` 包,只调 services / managers |
| 11 | Redis 当数据库 | 所有状态都能从 PG 重建(底座 + 增量模型;评论列表回源 DB) |
| 12 | 对外暴露内部 `id` | 所有 RPC / response 用业务主键 `post_id` / `comment_id` |

---

## 18. 附录:核心 SQL 速查

**发帖事务**(伪 SQL,实际 SQLAlchemy 三次单表 INSERT):
...
**点赞 upsert**(`post_likes` repository):
...
> 2. `session.execute(select(PostStat).where(PostStat.post_id.in_(post_ids)))` 批量拿计数
> 3. service 层内存 join 后再打分
>
> 这才符合「单表 repository + service 拼装」红线 1。
