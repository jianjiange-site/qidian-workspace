"""CommentFlushJob：每分钟把 Redis 评论增量批量刷盘到 post_stats。

设计背景（来自 post-service-design.md §6.2）:
- 与 LikeFlushJob 共用同一套 Write Coalescing 机制。
- 评论接口只 INCR Redis 增量；本 Job 每分钟聚合后批量刷盘到 post_stats.comment_count。

执行流程：
1. 从 Redis Set 中随机取出 100 个「有待刷盘增量」的 post_id。
2. 对每个 post_id，原子性地取走评论增量（Lua GET + SET 0）。
3. 在数据库事务中把增量累加到 post_stats.comment_count。
...
class CommentFlushJob:
    ...
    @distributed_lock("CommentFlushJob", lock_seconds=120)
    async def run(self) -> None:
        """Job 入口，由 APScheduler 每分钟调度一次。"""
        # 1. 取出这分钟有变动的 100 个 post_id
        post_ids = await PostStatManager(None, self._redis).pop_updated_post_ids(100)
        if not post_ids:
            return

        # 2. 对每个 post_id，原子取走评论增量并刷盘
        flushed = 0
        for post_id in post_ids:
            delta = await PostStatManager(None, self._redis).flush_comment_delta(post_id)
            if delta == 0:
                continue
            async with get_session() as session:
                stat_manager = PostStatManager(session, self._redis)
                await stat_manager.persist_comment_delta(post_id, delta)
            flushed += 1
        ...
        logger.info("Comment flush completed, processed=%s flushed=%s", len(post_ids), flushed)
